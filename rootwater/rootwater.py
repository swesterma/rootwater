...
.. note::
    2024-08-20: adapted to Python/3.10.8 and newer pandas releases (v2.2.2) by
    Westermann, S.. Input time series (ts) essentially need to have a timezone
    defined.
...
# helper
def nearby(ts, tx):
    return np.argmin(np.abs((ts - tx).seconds + (ts - tx).days * 86400.0))
    # likely never used...
...
def fRWU(
    ...
    RWU = pd.DataFrame(
        np.zeros((len(ddx), 10)) * np.nan, dtype="object"
    )  # changed (SW)
    ...
    def startstopRWU(dd):
        ...
        try:
            ...
            stop2RWU = tsx.index[
                np.where(tsx.values > 0)[0][
                    np.where(tsx.values > 0)[0]
                    > np.where(tsx.values <= -0.01)[0][
                        np.where(tsx.values <= -0.01)[0]
                        > np.where(tsx.values >= 0)[0][0]
                    ][0]
                ][0]
            ]
            reflim = np.min(
                [
                    -0.001,
                    tsx.loc[stopRWU:stop2RWU][
                        tsx.loc[stopRWU:stop2RWU] < -0.001
                    ].quantile(0.95),
                ]
            )
            startRWU = tsx.loc[stopRWU:].index[
                tsx.loc[stopRWU:].rolling(3, center=True).mean() <= reflim
            ][0]
            ...
            try:
                ...
                startRWU = tsx.index[
                    nearby(tsx.index, sunr(dd) + datetime.timedelta(hours=2))
                ]
                stop2RWU = tsx.index[
                    nearby(tsx.index, suns(dd) + datetime.timedelta(hours=1))
                ]
    ...
    def idstep_startstop(dd):
        ...
        try:
            ...
            tix = ts.index[
                ts.index.get_loc(
                    suns(dd) - datetime.timedelta(hours=0.5), method="nearest"
                )
            ]
        except:
            tin = ts.index[
    ...
    def dayRWU(dd):
        ...
        if ((tout - tin).seconds < mintime * 3600.0) | (
            (tix - tout).seconds < mintime * 3600.0
        ):
        ...
        df2 = pd.DataFrame(
            [np.arange(len(ts.loc[tout:tix])), ts.loc[tout:tix].values]
        ).T
        df2.columns = ["x", "y"]
        try:
        ...
        fuse = pd.Series(
            data=res.params.Intercept + res.params.x * np.arange(len(dummy)),
            index=dummy,
        )
        ...
        step_control = 0
        if (
            res.params.x / ((6.0 * 3600.0) / freqx.index[0].seconds) > -0.5 / 6.0
        ):  # night slope shall be more than minus 0.5 vol.% per 6h
            step_control += 10
        if (
            res.params.x / ((6.0 * 3600.0) / freqx.index[0].seconds) < 1 / 6.0
        ):  # night slope shall be less than plus 1 vol.% per 6h
            step_control += 100
        if (res2.params.x < 0) & (
            res2.params.x / ((6.0 * 3600.0) / freqx.index[0].seconds) > -0.5 / 12.0
        ):  # day slope must be negative but more than minus 0.5 vol.% per 12h
            step_control += 1000
        if (
            res2.params.x < slope_diff * res.params.x
        ):  # day slope must be at least 3 times more steep than night (if night was negative)
            step_control += 1
        if fuse.loc[tix] - ts.loc[tix] < 2.0:  # rwu should not exceed 2 mm/day
            step_control += 10000
    ...
    def dayRWU2(dd, crit_nse=0.5):
        ...
        dummyx = pd.concat(
            [
                ...
                ts.loc[
                    tin
                    - datetime.timedelta(hours=0.5) : tix
    ...
    for dd in ddx[:-1]:
        try:
            RWU.loc[pd.Timestamp(dd)] = dayRWU2(dd)  # changed (SW)
        except:
...
def dfRWUc(
    ...
    if safeRWU:
        dummz.loc[dummz.step_control < 11111, "rwu"] = (
            np.nan
        )  # refuse values based on too much night increase and no day decrease
        dummz.loc[dummz.rwu < 0.0, "rwu"] = np.nan  # refuse values less than zero
    dummx = dummz.rwu

    if safeRWU:
        dummz.loc[dummz.step_control < 11111, "rwu_nonight"] = (
            np.nan
        )  # refuse values based on too much night increase and no day decrease
        dummz.loc[dummz.rwu_nonight < 0.0, "rwu_nonight"] = (
            np.nan
        )  # refuse values less than zero
